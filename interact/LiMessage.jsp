                    ...
                    <div class="conB">
                        <div class="wrapB" style="width:100%;"/>
									...
									<div class="ListC rightL" >
										<div class="sp" style=""/><span class="nameW" id="userName">张三</span><span class="zuW shadow" id="groupName">群组一</span></div>
		...
		<div  class="col-sm-12 rightW" >
			<div class="imageUp fistS">
				...
				<div  id="edi">
				  ...
				  <form id="groupMessage" name="groupMessage" method="post" action="#" class="form-horizontal">
				  <input type="reset" name="reset" style="display: none;" />       
					...
					<div class="form-group seleImg" style="display:none;">
						<div >
							<div id="imagesDiv" ></div>
							<div >
								<button id="images" type="button" 
								        class="btn bgm-teal btn-block comp" 
								        comp="type:'fileupload',imagesDiv:'imagesDiv',isPrivate:true,showImage:true,imgWidth:'100px',imgHeight:'80px'">选择图片</button>
							</div>
						</div>
					</div>
						...
						<div class="sL">
							<select class="form-control" style=" -webkit-appearance:normal!important;appearance:normal!important;" data-live-search="true" id="groupId" name="groupId" required>
								...
								<c:forEach items="${favorgroups}" var="favorgroup">
									<option value="${favorgroup._id}">${favorgroup.name}</option>
								</c:forEach>
							</select>
						</div>
					</div>
...
$().ready(function() {
	$("#groupMessage").validate({
		submitHandler : function(form) {
			...
			$.ajax({
	  	        type:"post",
	  	        url:_ctxPath+"/group/sendMessage.action",
	  	      	data:$('#groupMessage').serialize(),
	  	        dataType:"json",
	...
	$("#cancelBtn").click(function() {
		document.location.href = _ctxPath + "/group/groupMessage/toQuery.do?groupId=${param.groupId}";
	});
});
...
function getNewsx(){
  ...
  $.ajax({
      type:"post",
      async:false,
      url:"messageList.action",
      data:{'page': currentPageAllAppointx,'size':limitAllAppointx,'groupId':'${groupId}'},
      success:function(data){
...
function getShopCustomesInfox(data){
  ...
  $.each(data,function(v,o){
      ...
      if('${xduser._id}'==o.userId){
		  ...
		  dels="<i class='ace-icon fa icon-jubao1  iconfont  fa-2x purple'><span><a href='javascript:void(0);' onclick='parent.openMoniFrom(4,\""+o._id+"\")' >举报</a></span></span></i>";
	  }
	  hf="<a href='javascript:void(0);' onclick=\"oncom('"+o._id+"')\">评论</a></span>";
	  s+='<li><div class="wrapB"><div class="ListC leftL">'
	      +simg+
	  '</div><div class="ListC rightL"><div class="sp"><span class="nameW">'
      ...
      '</p><div class="imgB">'
       ...
       '<div class="grid3 huoyJ" style="padding-left:10px;padding-right:10px;">'+
       '<i class="ace-icon fa icon-pinglun iconfont fa-2x "  ><span>'+
         hf+'('+o.commentTimes+')</span></i>'+
       ...
       '</div><div class="grid3 stuJ" style="padding-left:10px;padding-right:10px;"></div><div class="grid3 myJ" style="padding-left:10px;padding-right:10px;">'+dels+'</div></div></li>'
...
function getComments(){
  ...
  $.ajax({
      ...
      data:{'page': currentPageAllAppoint,'size':limitAllAppoint,
    	  'parentId': "0",'type|integer': 80,'typeId': vid},
      success:function(data){
				...
				function() {
					...
					if(c != '' && c.length> 0) {
						  $.ajax({
						      ...
						      data:{'type': 80,'typeId':vid,'content': c},
						      success:function(data){
									$("#form-field-1").val('');
									getComments();
								    toPage();
