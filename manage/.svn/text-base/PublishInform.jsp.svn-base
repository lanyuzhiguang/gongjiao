                    ...
                    <div class="row">
                        <div class="col-xs-12 wrapB">
                            <div class="tabbable">
                                ...
                                <c:if test="${ffuser.userrole.addnotice == 'addnotice'}">
                                    <li class="addN">
                                        ...
                                        <div class="col-sm-12 tableB">
                                            <table border="0">
                                                ...
                                                <tr id="itms1" style="display: none;">
                                                    <td style="text-align: left;"><a target="_blank"  id="noticeTitle1">标题标题标题标题标题标题标题标题</a></td>
...
//            }
        ...
        $("#newNotice").click(function() {
			document.location.href = "../notice/toInsert.action";
		});
        
        $("#groupTable")
		.grid(
				{
					url :"../notice/query.action",
					postData : $("#searchForm")
							.formobj(),
					shrinkToFit : false,
					colNames : [ "操作","通知类型", "标题", "单位","开始时间","地点","阅读数" ,"出勤率",
					             "发布时间"],
					colModel : [
							{
								name : "_id",
										...
										if(rowObject.delMeet!=-1){
											...
											if((new Date().getTime()-rowObject.attendtime)>0){
												if(rowObject.minType==0){
													v += "<a style='width:122px;display:inline;padding-left: 0px;' href='../noticeDetail/toInstTo.action?noticeId="
														+cellvalue+"' >上传会议资料</a>&nbsp;&nbsp;";
												}
											}
										}else{
							...
							},
							{
								...
								formatter : function(
									...
									if(cellvalue==undefined){
										...
										if(rowObject.notType==0){
									        v = "<a href='../noticeDetail/toDetail.action?noticeId="+rowObject._id
												+"' class='m-r-10' target=\"_blank\" >"+cellvalue+"</a>";
						        		}else if(rowObject.notType==1){
						        			v = "<a href='../noticeDetail/toMeeting.action?noticeId="
							...
							},
							{
								...
								formatter : function(
										...
										options,
										rowObject) {
									var v = "<a href='../noticeDetail/toQuery.action?noticeId="
								...
								formatter : function(
										...
										if(rowObject.delMeet!=-1){
											if(cellvalue==undefined){
												...
												v += "<a href='../noticeDetail/toReachQuery.action?noticeId="
													+rowObject._id+"' class='m-r-10'>"+cellvalue+"</a>";
											}
										}
										return v;
    ...
    function remove(obj) {
			layer.confirm('确定要撤销通知?', function(index) {
				layer.close(index);
				var url = "../notice/delNotice.action";
    ...
    function removeNotice(obj){
    	layer.confirm('删除该通知，收到该通知的用户将收不到删除提醒，且删除之后将不再有任何形式的显示，确定要删除该通知？', function(index) {
			layer.close(index);
			var url = "../notice/delAllNoticeAndDetail.action";
			$.get(url, {'noticeId':obj}, function(data) {
				if (data) {
					if (data.success) {
						toastr.success("操作成功");
      ...
      //分页参数设置 这些全局变量关系到分页的功能
        ...
        function getNoticeItm1(data){
        	...
        	for(var i=0;i<data.length;i++){
        		...
        		if(notd.notType==0){//=\"\"
	        		listItem.find("#noticeTitle1").attr('href', "../noticeDetail/toDetail.action?noticeId="+notd.noticeId);
        		}else if(notd.notType==1){
        			listItem.find("#noticeTitle1").attr('href', "../noticeDetail/toMeeting.action?noticeId="+notd.noticeId);
        		}
        		...
        		if(notd.state==0){
        			...
        			listItem.find("#state1").html('通知已撤销');
        		}
   ...
   function flasPgs(){
	   window.location.href="../noticeDetail/toList.action";
   }     	
