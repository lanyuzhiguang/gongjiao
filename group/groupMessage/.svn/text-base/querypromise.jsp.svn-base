...
<body class="gray-bg">
				...
				<form id="searchForm" method="get" class="form-horizontal">
					...
					<div class="col-sm-3">
						...
						<div class="fg-line">
					 	...
					 	<div class="col-sm-2">
						<div class="input-group fg-float">
						 <c:if test="${fftenant.promise == 0}">
						<div class="fg-line">
					    	<button id="pubpromise" class="btn btn-primary" onclick="dsetrec()" type="button">开启发布</button>
					    </div>
					      </c:if>
					       <c:if test="${fftenant.promise == 1}">
					    <div class="fg-line">
					    	<button id="pubpromise" class="btn btn-primary" onclick="dsetrec()" type="button">关闭发布</button>
					    </div>
					    </c:if>
					    </div>
					   </div>
	...
	startDate.config.max.date = '';
				...
				$("#messageTable")
					...
					{
						url : _ctxPath
								+ "/pc/group/groupMessagePc/query.action?groupId=0",
						postData : $("#searchForm")
								.formobj(),
						shrinkToFit : false,
						colNames : [ "操作", "内容","发布人", "状态",
								"发布时间" ],
						colModel : [
								{
									name : "_id",
									...
									formatter : function(
											...
											options,
											rowObject) {
										var v = "<a href='${ctxPath}/pc/group/groupMessagePc/toUpsertpromise.action?groupId=0&id="
												+ cellvalue
												+ "' class='m-r-10'>详情</a>";
										return v;
									}
		...
		function dsetrec() {
			var url=_ctxPath + "/basic/tenant/promiseset.do";
