		...
		this.each(function() {
			...
			for(var tb in toolbar_buttons) if(toolbar_buttons.hasOwnProperty(tb)) {
				...
				switch(button.name) {
					...
					case 'insertImage':
						toolbar += ' <div class="btn-group"><a class="btn btn-sm '+className+' dropdown-toggle" data-toggle="dropdown" title="'+button.title+'"><i class="'+ ace.vars['icon'] + button.icon+'"><span>图片</span></i></a> ';
						toolbar += ' <div class="dropdown-menu dropdown-caret dropdown-menu-right">';
			...
			toolbar += ' </div><span class="selectB"><select><option value="manage">公开</option><option value="peop">peop</option></select><a class="publish shadow">发布</a><a class="repBox shadow">回复</a></span>';
			////////////
			var speech_input;
			...
			else toolbar = $(this).after(toolbar).next();
