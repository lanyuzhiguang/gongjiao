...
<head>
    ...
    <link href="${ctxPath}/modulepc/css/audioDetail.css" rel="stylesheet"/>
</head>

<body onunload="score()" class="no-skin" oncontextmenu=self.event.returnValue=false onselectstart="return false">
                ...
                <div class="col-xs-12 wrapB">
                    <div class="conB">
                        ...
                        <div class="imgW">
                          ...
                          <audio  controls="controls" id="srcurl2" >
                        ...
                        <div class="hr hr32 hr-dotted"></div>
                               <div class="clearfix" id="area">
                                    ...
                                    <div class="grid3 huoyJ" style="width:100px">
                                        <i class="ace-icon fa iconfont fa-2x" id="zan" ><span >点赞(0)</span></i>
                                    </div>
                                    <div class="grid3 huoyJ"  style="width:100px">
                                        <i class="ace-icon fa iconfont fa-2x" id="favor"><span  >收藏(0)</span></i>
                                    </div>
                                    <!--  
                                    <div class="grid3 stuJ">
                                        <i class="ace-icon fa icon-jubao1  iconfont  fa-2x purple"><span>举报</span></i>
                                    </div>-->
...
function score() {
	...
	if(tc>newdata.duration&&newdata.duration>0) tc=newdata.duration;
	if(tc > 0) {
		$.ajax({
		      type:"post",
		      async:false,
		      url:"../score/studyTimeScore.action",
		      data:{'type': '2','typeId':newdata._id,'time': tc,'libSore': 0},
...
function getNewsById(){
	...
	if(type != null && type != ""){
		data = {'studyId':'${id}','type':'1'};
		$("#area").hide();
	}else{
		data = {'studyId':'${studyId}','type':'2'};
	}  
	
	$.ajax({
	      type:"post",
	      async:false,
	      url:"queryStudyById.action",
	      data:data,
	      success:function(data){
	    	  data = data.data;
	    	  newdata=data;
			    ...
			    document.getElementById("srcurl2").src = data.fileidUrl;
  ...
  $.ajax({
     ...
     // async:false,
      ...
      data:{'page': currentPageAllAppoint,'size':limitAllAppoint,
    	  'parentId': "0",'type|integer': 2,'typeId': '${studyId}'},
      success:function(data){
				...
				function() {
					...
					if(c != '' && c.length> 0) {
						  $.ajax({
						      ...
						      data:{'type': 2,'typeId':'${studyId}','content': c},
						      success:function(data){
									$("#form-field-1").val('');
									getComments();
								    toPage();
