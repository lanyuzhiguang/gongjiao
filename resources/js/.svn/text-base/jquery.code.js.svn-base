...
(function($) {
		...
		$(".codecfg", this).add(this).each(
				function(i) {
					...
					if (!tag)
						return;
					tag = tag.toLowerCase();
					var codecfg = options.codecfg ? options.codecfg : $(this)
							.attr("codecfg");
					if (codecfg) {
						var cfgObj = $.codecfgobj(this, codecfg);
						if (options.tags && !options.tags.contains(tag)
								&& !cfgObj.render)
							return;
						arr.push(cfgObj);
					}
	...
	$.fn.codetext = function(options) {
		...
		$(".codecfg", this).add(this).each(
				function(i) {
					...
					if (tag != "select") {
						var codecfg = options.codecfg ? options.codecfg : $(
								this).attr("codecfg");
						if (codecfg) {
							var cfgObj = $.codecfgobj(this, codecfg);
							if (cfgObj.render)
								return;
							var text = tag == "input" ? $(this).val() : $(this)
									.text();
							if (text) {
								/*
								 * var valueField = cfgObj["valueField"] ?
								 * cfgObj["valueField"] : "code_value"; var w =
								 * valueField + "='" + text + "'";
								 */
								cfgObj['_id'] = text;
								arr.push(cfgObj);
	...
	$.codecfgobj = function(ele, codecfg) {
		if (codecfg) {
			...
			var cfgObj = $
					.parseJSON(idx == 0 ? codecfg : ('{' + codecfg + '}'));
			var eid = ele.name ? ele.name : ele.id;
	...
	$.genoption = function(arr, options) {
		if (arr.length > 0) {
			...
			$.ajax({
				type : "POST",
				url : _ctxPath + "/i/code/selectCode.do",
				async : false,
				data : {
					jsonPara : jsonStr
				},
							...
							.each(function() {
								...
								function renderRadio(t, codeOptions, f) {
									...
									for ( var opt in codeOptions) {
										...
										// OA-27642
										// 修复html方式弹出dialog时生成的radio没有name的问题
										var rd = $('<input class="radio_com" type="radio" name="'
												+ id + '">');
										rd.attr("id", id);
										// rd.attr("name", id);
										rd.val(opt);
	...
	$.findTag = function(obj) {
		var ele;
		if (obj.eleid) {
			ele = $("[name='" + obj.eleid + "']").length == 0 ? $("#"
					+ obj.eleid) : $("[name='" + obj.eleid + "']");
		}
		if (!ele) {
			$(".codecfg")
					.each(
