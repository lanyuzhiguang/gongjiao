...
//$(function(){
    ...
    $(".answerallnum").on("input propertychange",function(){
    ...
    $(".singleElection,.multiSelection,.judgmentProblem,.completion").on("input propertychange",function(){
		...
		$(".currentTotalScore").text(String(totalScore));
    })
...
//  	}
	...
	//数据回显
	if (editType == "1") {
		//choseQuestionBank = JSON.parse(localStorage.getItem("pagerData"));
		//examStrs = choseQuestionBank.join(",");
		$.ajax({
			type:"get",
			url:path + "pc/courseExamPaperPc/findByBaseId.action",
			xhrFields: {				
	        ...
	        dataType: "json",
			data:{
				"baseId": localStorage.getItem("id")
			},
			success: function(data){
				var pagerDataAll = data.data;
				//console.log(pagerDataAll);
				pagerId = pagerDataAll._id;
				$(".selectlist").attr("title",pagerDataAll.mode);
				$(".title").val(pagerDataAll.title);
				$(".selectlist").val($(".selectlist option").eq(pagerDataAll.mode-1).text());
				choseQuestionBank = pagerDataAll.examId;
				examStrs = choseQuestionBank.join(",");
				$(".branchSetting input").removeAttr("readonly");
				textPargerType();
//				if (pagerDataAll.mode == "2") {
//					$(".operationAttr").attr("readonly","readonly");
//				} else {
//					$(".operationAttr").removeAttr("readonly");
//				}
				function moType(data){
					return data == undefined ? 0 : data;
				}
				$(".altogetherNum:eq(0)").text(pagerDataAll.singleElectionSum);
				$(".altogetherNum:eq(1)").text(moType(pagerDataAll.multiSelectionSum));
				$(".altogetherNum:eq(2)").text(moType(pagerDataAll.judgeSum));
				$(".altogetherNum:eq(3)").text(moType(pagerDataAll.fillAVacancySum));
				$(".rdsubjectNumText").text(moType(pagerDataAll.singleElectionSum)+moType(pagerDataAll.multiSelectionSum)+moType(pagerDataAll.judgeSum)+moType(pagerDataAll.fillAVacancySum));
				qbsingleElection = moType(pagerDataAll.singleElectionSum);
				qbmultiSelection = moType(pagerDataAll.multiSelectionSum);
				qbcompletion = moType(pagerDataAll.judgeSum);
				qbjudgmentProblem = moType(pagerDataAll.fillAVacancySum);
				$(".singleElection").val(moType(pagerDataAll.singleElection));
				$(".singleElectionFraction").val(moType(pagerDataAll.singleFraction));
				$(".multiSelection").val(moType(pagerDataAll.multiSelection));
				$(".multiSelectionFraction").val(moType(pagerDataAll.multiFraction));
				$(".missedElectionFraction").val(moType(pagerDataAll.missingFraction));
				$(".judgmentProblem").val(moType(pagerDataAll.judge));
				$(".judgmentProblemFraction").val(moType(pagerDataAll.judgeFraction));
				$(".completion").val(moType(pagerDataAll.fillAVacancy));
				$(".completionFraction").val(moType(pagerDataAll.fillFraction));
				var hxtotalScore = pagerDataAll.singleElection*pagerDataAll.singleFraction+pagerDataAll.multiSelection*pagerDataAll.multiFraction+pagerDataAll.judge*pagerDataAll.judgeFraction+pagerDataAll.fillAVacancy*pagerDataAll.fillFraction;
				$(".totalScore").text(hxtotalScore);
				$(".currentTotalScore").text(hxtotalScore);
				$(".timeLength").val(pagerDataAll.answerTime);
				$(".qualified").val(pagerDataAll.qualfraction);
				$(".about").val(pagerDataAll.about);
				$(".answerallnum").val(pagerDataAll.sumfrequency)
				$("label#showtime").attr("showtime",pagerDataAll.showTime);
				if (pagerDataAll.showTime == 1) {
					$("#showtime").addClass("switchActive");
				} else{
					$("#showtime").removeClass("switchActive");
				}
				$("label#distinct").attr("distinct",pagerDataAll.distinct);
				if (pagerDataAll.distinct == 1) {
					$("#distinct").addClass("switchActive");
				} else{
					$("#distinct").removeClass("switchActive");
				}
			}
		});
	}
	...
	var userArr = [];
		...
		$("#showtime").on("click",function(){
			if ($(this).attr("showTime") == 0) {
				...
				$(this).removeClass("switchActive");
			}
			
		})
		//是否题目相同
		$("#distinct").on("click",function(){
			if ($(this).attr("distinct") == 0) {
				$(this).attr("distinct","1");
				$(this).addClass("switchActive");
			} else{
				$(this).attr("distinct","0");
				$(this).removeClass("switchActive");
			}
...
//			console.log($(".currentTotalScore").text());
			...
			if ($(".title").val() == "") {
				$(".prompt").fadeIn();
				$(".promptText").text("请填写试卷名字！");
			} else if ($(".subjectNumText").text() == "0"&&$(".rdsubjectNumText").text() == "0") {
				$(".prompt").fadeIn();
				$(".promptText").text("请选择题目！");
			} else if($(".totalScore").text() == "0") {
				$(".prompt").fadeIn();
				$(".promptText").text("请设置题目分数！");
			} else if($(".timeLength").val() == ""){
				$(".prompt").fadeIn();
				$(".promptText").text("请填写答题时长！");
			} else if($(".qualified").val() == ""){
				$(".prompt").fadeIn();
				$(".promptText").text("请填写合格分数！");
				...
				$.ajax({
					type: "post",
		            url:path+"pc/courseExamPaperPc/upsert.action",//"http://192.168.3.3:80/dangjian"
		            xhrFields: {				
			            withCredentials: true	
			        },							
			        crossDomain: true,
		            data:{
		            	...
		            	baseId: id,
		            	_id: pagerId,
		            	distinct:$("#distinct").attr("distinct"),
		            	sumfrequency:$(".answerallnum").val()
		            },
					success:function(data){
						if(data.success == true){
							localStorage.setItem("examSetting",true);
							var course3 = JSON.parse(localStorage.getItem("course3"));
							course3.examName = $(".title").val();
							localStorage.setItem("course3",JSON.stringify(course3));
							window.history.go(-1);
						} else{
							$(".prompt").fadeIn();
