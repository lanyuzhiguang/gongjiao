...
<html>
		...
		<div class="card">
			...
			<c:if test="${account.entityID != ffuser._id }">
					<div class="my-titlebar">
						<a href="${ctxPath}/bumen/toQuery.do?parentId=${param.parentId}"
					...
					<div class="form-group">
						<div class="col-sm-4 col-sm-offset-2">
							...
							<button id="cancelBtn" class="btn bgm-gray m-l-20" type="button">取消</button>
										...
										<div id="faq-tab-2" class="tab-pane fade col-sm-12"><!-- 组织奖惩 -->
										...
										<div id="faq-tab-3" class="tab-pane fade col-sm-12"><!-- 班子成员 -->
										...
										<div id="faq-tab-4" class="tab-pane fade col-sm-12"> <!-- 所在单位信息 -->
	...
	.ready(
			function() {
				...
				$("#cancelBtn")
						.click(
								function() {
									...
									+ "/bumen/toQuery.do?parentId=${param.parentId}";//因为由于租户id在树中是0所以获取上一级参数
								});

				$("#addOutcome").click(
						function() {
							document.location.href = _ctxPath
									+ "/outcome/toUpsert.do?type=0&bumenId=${ffbumen._id }";//当前所需要的是正在编辑的部门的id所以使用ffbumen._id 获取
						});
				$("#addInstitution").click(
						function() {
							document.location.href = _ctxPath
									+ "/institution/toUpsert.do?bumenId=${ffbumen._id }";//当前所需要的是正在编辑的部门的id所以使用ffbumen._id 获取
						});
				...
				$.post(
						'${ctxPath}/division/queryProvince.do',
						{},
				...
				.change(
						function() {
							...
							if (v && v.tmp) {
								$
										.post(
												'${ctxPath}/division/queryDistrict.do',
												{
													cityId : v.tmp
												},
												function(
	...
	function changCategory(obj){
		document.location.href = _ctxPath + "/outcome/toUpsert.do?outcomeId="+obj;
	}

	function removeCategory(obj) {
		layer.confirm('该条奖惩信息删除后将无法恢复，是否确认删除？',{title:"删除"}, function(index) {
			layer.close(index);
			var url = _ctxPath + "/outcome/del/" + obj + ".do";
	...
	function removeLeadershipCategory(obj) {
		layer.confirm('该条信息删除后将无法恢复，且对应的班子成员信息也将删除，请谨慎确认是否要进行删除操作？',{title:"删除"}, function(index) {
			layer.close(index);
			var url = _ctxPath + "/leadership/delLeadership/" + obj + ".do";
			$.get(url, {}, function(data) {
				if (data) {
					if (data.success) {
						toastr.success("操作成功");
	...
	function changinst(obj){
		document.location.href = _ctxPath + "/institution/toUpsert.do?instId="+obj;
	}

	function changLeadershipCategory(obj){
		document.location.href = _ctxPath + "/leadership/toUpsert.do?leadershipId="+obj;
	}
		...
		layer.open({
			...
			yes: function(index, layero){
			    ...
			    if($.trim(num)!=""){
			    	...
			    	$.ajax({
			  	        type:"post",
			  	        url:_ctxPath+"/leadership/upsert.do",
	...
	function removeinst(obj) {
		layer.confirm('该条信息删除后将无法恢复，是否确认删除？',{title:"删除"}, function(index) {
			layer.close(index);
			var url = _ctxPath + "/institution/del/" + obj + ".do";
			$.get(url, {}, function(data) {
				if (data) {
					if (data.success) {
						toastr.success("操作成功");
