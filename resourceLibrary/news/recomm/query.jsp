					...
					<div class="col-sm-12">
						<form id="searchForm" method="get" class="form-horizontal">
							<div class="form-group">
								...
								<input type="hidden" id="state|integer" name="state|integer" value="1">
								...
								<div class="col-sm-2">
									<select class="form-control selectpicker" id="istoLib|integer"
										name="istoLib|integer" data-live-search="true">
										<option value="">请选择</option>
										<option value="-1">推荐未采纳</option>
										<option value="0">未推荐</option>
										<option value="1">推荐审核中</option>
										<option value="2">推荐已采纳</option>
									</select>
								</div>
								</c:if>
								<div class="col-sm-1">
	...
	<script type="text/javascript">//hasNews 新闻运营判断
									...
									.click(
											function() {
												document.location.href = _ctxPath
														+ "/pc/newsPc/toAudUpsert.action?categoryId="
														+ categoryId;
											});
									...
									.grid(
											{
												...
												colNames : [ "操作", "标题","发布日期", 
												        "新闻栏目","发布者",/* "状态" ,*/ "是否置顶",
														"点击次数", "收藏次数", "赞次数"
														<c:if test="${tenant.hasNews == 1}">, "推荐状态" </c:if>],
												colModel : [
														{
															name : "_id",
															index : "_id",
															...
															formatter : function(
																...
																var v = "";
																<c:if test="${tenant.hasNews == 1}">
																if (rowObject.istoLib == 0 || rowObject.istoLib == -1) {
																	if (rowObject.state == 1) {
																		v += "<a href='javascript:void(0);' onclick='toLib(\""
																			+ cellvalue
																			+ "\")' class='m-r-10'>推荐</a>";
																	}
																}
														...
														},
														{
															...
															formatter : function(
																	...
																	options,
																	rowObject) {
																var v = "<a href='${ctxPath}/pc/newsPc/toAudUpsert.action?newsId="+rowObject._id+"' >"+cellvalue+"</a>";
																return v;
															}
														},
														{
		...
		function toLib(newsId){
			layer.confirm('确定推荐到资源库吗?', function(index) {
				layer.close(index);
				var url = _ctxPath + "/pc/newsPc/toLib/" + newsId + ".action";
				$.get(url, {}, function(data) {
					if (data) {
						if (data.success) {
							toastr.success("操作成功");
		...
		$.ajax({
  	        type:"post",
  	        url:"${ctxPath}/pc/news/categoryPc/queryAllByTenantId.action",
  	        dataType:"json",
