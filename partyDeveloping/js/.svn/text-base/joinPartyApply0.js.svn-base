...
function getJoinParyList0(){
	$.ajax({
		   type:"post",
		   url:_ctxPath+"/recruit/queryAdmin.action",
		   data:{'page': nowpagea,'size':sizea,'type|integer':5},
		   dataType:"json",
...
function setJoinParyList0(data){
	var party0=$("#party0");
	party0.html("");
	for (var i = 0; i < data.length; i++) {
		...
		party0.append(" <tr> <td>"+userName+"</td> <td>"+gender+"</td> <td><span>"+bumenName
				+"</span></td> <td>"+crtTime+"</td> <td>"+
				"<a class=\"look\" href='applyerInfo.html?recId="+recruit._id+"'>查看</a>"
				+"</td> </tr>");
	}
	if(data.length<1){
